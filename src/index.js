...
async function getRecommendedMenu(location, date, weather) {
  ...
  const promptForm = `출력 형식은 다음과 같이 출력해. 1. 시간에 어울리는 문구를 {suggestedFoodText}에 점심이면 "오늘의 점메추는" 저녁이면 "오늘의 저메추는"으로 시작해서 짧은 문장을 작성해. 2. {weatherCondition}에는 현재 날씨를 문구로 나타내. 3. 위의 내용을 참고해서 다음 마크다운 문법으로 출력해. # 오늘 뭐 먹지?</br>yyyy년 d월 d일 {hour}시 {minutes}분 {location}의 날씨는 {temperature}℃이고, {weatherCondition}. {suggestedFoodText}.</br>1. **메뉴명1**: 이유1</br>2. **메뉴명2**: 이유2</br>3. **메뉴명3**: 이유3</br>`;
